    ...
    async def game_top(self, ctx: MyContext):
        ...
        for i, user in enumerate(top_vaccines):
            if user.player.discord_id not in self.bot.owner_ids:
                embed.add_field(name=f"#{i}: {user.player.discord_name}", value=f"{user.made_vaccines}", inline=False)
                user_made_vaccine = True
            else:
                embed.add_field(name=f"#{i}: Bot Admin", value=f"{user.made_vaccines}", inline=False)
