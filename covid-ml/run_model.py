...
while True:
    ...
    try:
        ...
        # Deprecated
        # df_recovered = pd.read_csv('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/'
...
def plot_params(_ax, axis_label=None, plt_title=None, label_size=15, axis_fsize=15, title_fsize=20, scale='linear'):
    # Tick-Parameters
    _ax.xaxis.set_minor_locator(ticker.AutoMinorLocator())
    _ax.yaxis.set_minor_locator(ticker.AutoMinorLocator())
    _ax.tick_params(which='both', width=1, labelsize=label_size)
    _ax.tick_params(which='major', length=6)
    _ax.tick_params(which='minor', length=3, color='0.8')
    ...
    plt.minorticks_on()
    # Plot Axes Labels
    plt.xlabel(axis_label[0], fontsize=axis_fsize)
    plt.ylabel(axis_label[1], fontsize=axis_fsize)


# noinspection PyUnusedLocal
def visualize_covid_cases(confirmed, deaths, _continent=None, country=None, state=None, period=None, figure=None,
                          scale="log"):
    _x = 0
    if figure is None:
        ...
        _f = figure[0]
        # Sub plot
        _ax = _f.add_subplot(figure[1], figure[2], figure[3])
    _ax.set_axisbelow(True)
    plt.tight_layout(pad=10, w_pad=5, h_pad=5)

    _stats = [confirmed, deaths]
    _label = ["Confirmed", "Deaths"]

    if _continent is not None:
        params = ["continent", _continent]
    elif country is not None:
        params = ["country", country]
    else:
        params = ["All", "All"]
    color = ["darkcyan", "crimson"]
    _marker_style = dict(linewidth=3, linestyle='-', marker='o', markersize=4, markerfacecolor='#ffffff')
    for _i, _stat in enumerate(_stats):
        if params[1] == "All":
            cases = np.sum(np.asarray(_stat.iloc[:, 5:]), axis=0)[_x:]
        else:
            cases = np.sum(np.asarray(_stat[_stat[params[0]] == params[1]].iloc[:, 5:]), axis=0)[_x:]
        date = np.arange(1, cases.shape[0] + 1)[_x:]
        plt.plot(date, cases, label=_label[_i] + " (Total: " + format(cases[-1], ",") + ")", color=color[_i], **_marker_style)
        plt.fill_between(date, cases, color=color[_i], alpha=0.3)

    if params[1] == "All":
        total_confirmed = np.sum(np.asarray(_stats[0].iloc[:, 5:]), axis=0)[_x:]
        total_deaths = np.sum(np.asarray(_stats[1].iloc[:, 5:]), axis=0)[_x:]
    else:
        total_confirmed = np.sum(np.asarray(_stats[0][_stat[params[0]] == params[1]].iloc[:, 5:]), axis=0)[_x:]
        total_deaths = np.sum(np.asarray(_stats[1][_stat[params[0]] == params[1]].iloc[:, 5:]), axis=0)[_x:]

    _text = "From " + _stats[0].columns[5] + " to " + _stats[0].columns[-1] + "\n"
    _text += "Mortality rate : " + format(int(total_deaths[-1] / (total_confirmed[-1]) * 10000) / 100, ",") + "\n"
    _text += "Last 5 Days:\n"
    _text += "Confirmed: " + format(total_confirmed[-1] - total_confirmed[-6], ",") + "\n"
    _text += "Deaths: " + format(total_deaths[-1] - total_deaths[-6], ",") + "\n"
    _text += "Last 24 Hours:\n"
    _text += "Confirmed: " + format(total_confirmed[-1] - total_confirmed[-2], ",") + "\n"
    _text += "Deaths: " + format(total_deaths[-1] - total_deaths[-2], ",")

    plt.text(0.02, 0.78, _text, fontsize=15, horizontalalignment='left', verticalalignment='top',
             transform=_ax.transAxes, bbox=dict(facecolor='white', alpha=0.4))

    # Plot Axes Labels
    axis_label = ["Days (" + df_confirmed.columns[5] + " - " + df_confirmed.columns[-1] + ")", "No of Cases"]

    # Plot Parameters
    plot_params(_ax, axis_label, scale=scale)
    ...
    def format_num(num: Union[int, float]) -> str:
        ...
        if num < 1000:
            ...
            num = "{:,.1f}m".format(num / 1000000)
        else:
            num = "{:,.1f}b".format(num / 1000000000)
        return num

    _ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda _x, pos: format_num(_x)))
...
def get_mortality_rate(confirmed, deaths, _continent=None, country=None):
    if _continent is not None:
        params = ["continent", _continent]
    elif country is not None:
        params = ["country", country]
...
print("done top_10_countries_confirmed.png")
...
print("done top_10_countries_deaths.png")
...
print("done top_10_countries_active.png")
...
print("done top_10_countries_recovered.png")
...
for i in range(0, len(df_confirmed)):
    ...
    folium.Circle(
        ...
        tooltip="<h5 style='text-align:center;font-weight: bold'>" + df_confirmed.iloc[i]['country'] + "</h5>" +
                ...
                "<li>Mortality Rate: " + str(np.round(df_deaths.iloc[i, -1] /
                                                      (df_confirmed.iloc[i, -1] + 1.00001) * 100, 2)) +
                "</ul>"
        ,
...
print("done worldmap_cases_interactive.html")
...
f.add_subplot(111)
marker_style = {'c': "crimson", 'linewidth': 6, 'linestyle': '-', 'marker': 'o', 'markersize': 8,
...
plt.close()
print("done countries_vs_date_spread.png")

cols = 1
rows = 1
f = plt.figure(figsize=(10, 10 * rows))

visualize_covid_cases(df_confirmed, df_deaths, _continent="All", figure=[f, rows, cols, 1])
...
for i, continent in enumerate(continents):
    visualize_covid_cases(df_confirmed, df_deaths, _continent=continent, figure=[f, rows, cols, i + 1])
...
for i, continent in enumerate(continents):
    f = plt.figure(figsize=(10, 10 * rows))
    visualize_covid_cases(df_confirmed, df_deaths, _continent=continent, figure=[f, rows, cols, 1])
    # visualize_covid_cases(df_confirmed, df_deaths, continent = continent,figure = [f,1,1, 1])
    name = continent.lower()
...
for i, country in enumerate(temp.index):
    ...
    x_new = np.linspace(date.min(), date.max(), 30)
    spl = make_interp_spline(date, t, k=1)  # type: BSpline
    power_smooth = spl(x_new)
    plt.plot(x_new, power_smooth, '-o', label=country, linewidth=3, markevery=[-1])
...
y = 2 ** (x + np.log2(threshold))
plt.plot(x, y, "--", linewidth=2, color="gray")
plt.annotate("# of cases doubles every day", (x[-2], y[-1]), xycoords="data", fontsize=14, alpha=0.5)
...
# noinspection PyArgumentList,PyArgumentList
x_new = np.linspace(date.min(), date.max(), 10)
spl = make_interp_spline(date, t, k=1)  # type: BSpline
power_smooth = spl(x_new)
...
plt.annotate("Num of Cases Doubles Every Day", (np.log2((t.max() - threshold) / threshold), t.max() - threshold / 2),
             xycoords="data", fontsize=14, alpha=0.5)

x = np.arange(0, days)
y = 2 ** (x / 3 + np.log2(threshold))
plt.plot(x, y, "--", linewidth=1, color="gray")
plt.annotate("... Every Third Day", (np.log2((t.max() - threshold) / threshold) * 3, (t.max() / 1.2) - threshold / 2),
             xycoords="data", fontsize=14, alpha=0.5)
...
y = 2 ** (x / 30 + np.log2(threshold))
plt.plot(x, y, "--", linewidth=1, color="gray")
plt.annotate("... Every Month", (len(t) - 25, 2 ** ((len(t) - 25) / 30 + np.log2(threshold))), xycoords="data",
             fontsize=14, alpha=0.5)

x = np.arange(0, days)
y = 2 ** (x / 13 + np.log2(threshold))
plt.plot(x, y, "--", linewidth=1, color="Red")
plt.annotate(".. Every 13 Days", (np.log2((t.max() - threshold) / threshold) * 13, t.max() - threshold), color="Red",
             xycoords="data", fontsize=14, alpha=0.8)
...
nextdays = [(datetime.strptime(d[-1], '%d %b') + timedelta(days=i)).strftime("%d %b") for i in
...
plt.text(0.02, 0.78, text, fontsize=17, horizontalalignment='left', verticalalignment='top', transform=ax.transAxes,
...
thoudand = 1000
temp = df_confirmed.groupby('country').sum().diff(axis=1).sort_values(df_deaths.columns[-1], ascending=False).head(
    10).replace(np.nan, 0)
threshold = 0
f = plt.figure(figsize=(20, 12))
ax = f.add_subplot(111)
...
thoudand = 1000
temp = df_deaths.groupby('country').sum().diff(axis=1).sort_values(df_deaths.columns[-1], ascending=False).head(
    10).replace(np.nan, 0)
threshold = 0
f = plt.figure(figsize=(20, 12))
ax = f.add_subplot(111)
...
stats = [df_covid19.loc[:, ['country', 'Confirmed']], df_covid19.loc[:, ['country', 'Deaths']],
         df_covid19.loc[:, ['country', 'Active']], df_covid19.loc[:, ['country', 'Recovered']]]
...
for i, stat in enumerate(stats):
    ...
    plt.pie(df_countries, labels=labels, autopct='%1.1f%%', pctdistance=0.85, labeldistance=1.2,
            textprops={'fontsize': 10.5})
    my_circle = plt.Circle((0, 0), 0.7, color='white')
    p = plt.gcf()
    p.gca().add_artist(my_circle)
    plt.text(0.5, 0.5, "World Total " + label[i] + " COVID-19 Cases\n" + str(stat.sum().values[1]),
             horizontalalignment='center', verticalalignment='center', transform=ax.transAxes, size=18, alpha=0.6)
...
df_data = df_table.groupby(['Last_Update', 'Country_Region'])[
    'Confirmed', 'Deaths', 'continent'].max().reset_index().fillna(0)
df_data["Last_Update"] = pd.to_datetime(df_data["Last_Update"]).dt.strftime('%m/%d/%Y')
